import os
import logging
import chess.pgn
from pgn_data_extraction import parse_thinking_time_from_comment

logger = logging.getLogger(__file__)


if __name__ == '__main__':
    dir_path = os.path.dirname(os.path.realpath(__file__))

    game_files = "ficsgamesdb_{}_standard2000_movetimes.pgn"
    csv_files = "ficsgamesdb_{}_standard2000_movetimes.csv"

    for date_string in list(range(2010,2018))+list((201801,)):
        input_file = os.path.join(dir_path, "games", game_files.format(date_string))
        output_file = os.path.join(dir_path, "data", csv_files.format(date_string))

        with open(input_file, 'r') as pgn_file:
            with open(output_file, 'w') as csv_file:

                processedGames = 0
                skippedGames = 0

                # Loop through games
                game = chess.pgn.read_game(pgn_file)
                while game:

                    # Skip games if not both players makes at least one move
                    if int(game.headers["PlyCount"]) < 2:
                        skippedGames += 1

                    else:
                        processedGames += 1
                        white_elo = game.headers["WhiteElo"]
                        black_elo = game.headers["BlackElo"]
                        time_control = game.headers["TimeControl"]
                        initial_time, extra_time = map(int, time_control.split('+'))

                        result = game.headers["Result"]
                        if (result == "1/2-1/2"):
                            result_value = 0.5
                        elif (result == "1-0"):
                            result_value = 1
                        elif (result == "0-1"):
                            result_value = 0
                        else:
                            raise ValueError("Result in PGN file is '{}' which is not supported.".format(result))

                        white_current_time = initial_time
                        white_total_time = initial_time
                        white_used_time = 0

                        black_current_time = initial_time
                        black_total_time = initial_time
                        black_used_time = 0

                        # Loop through moves in game
                        node = game
                        white_to_move = True
                        while not node.is_end():
                            node = node.variations[0]
                            thinking_time = parse_thinking_time_from_comment(node.comment)
                            if white_to_move:
                                white_used_time += thinking_time
                                white_total_time += extra_time
                                white_current_time = white_current_time - thinking_time + extra_time
                            else:
                                black_used_time += thinking_time
                                black_total_time += extra_time
                                black_current_time = black_current_time - thinking_time + extra_time

                            white_to_move = not white_to_move

                        # We've reached the end of the game
                        white_time_usage_ratio = white_used_time / white_total_time
                        black_time_usage_ratio = black_used_time / black_total_time

                        # (white_ELO, black_ELO, white_time_usage, black_time_usage, result)
                        csv_line = "{},{},{},{},{}\n".format(white_elo, black_elo, white_time_usage_ratio, black_time_usage_ratio, result_value)
                        csv_file.write(csv_line)

                    # Next game, possibly 'None'
                    game = chess.pgn.read_game(pgn_file)

        print("Processed {} games into {} (skipped {} games)".format(processedGames, output_file, skippedGames))
